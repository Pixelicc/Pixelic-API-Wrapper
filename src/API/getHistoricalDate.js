...
/**
 * Retrieve the stats of a specific player on the specified date (Returns the total stats from the last check).
 * @constructor
...
module.exports = async function (date, player) {
  ...
  const res = await this.makeRequest(`https://api.pixelic.de/player/historical/${date}/${player}`);
